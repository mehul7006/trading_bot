...
clear
echo -e "${CYAN}================================================================${NC}"
echo -e "${CYAN}          SSL CERTIFICATE FIX & MARKET DATA DOWNLOADER${NC}"
echo -e "${CYAN}================================================================${NC}"
echo ""
echo -e "${YELLOW}This script fixes SSL certificate issues on macOS and provides${NC}"
echo -e "${YELLOW}alternative data sources for Indian market data.${NC}"
echo ""

# Check if we're on macOS
if [[ "$OSTYPE" == "darwin"* ]]; then
    echo -e "${BLUE}Detected macOS - applying SSL certificate fixes...${NC}"
    
    # Method 1: Update certificates using Python
    echo -e "${YELLOW}[1/3] Updating Python SSL certificates...${NC}"
    python3 -c "
import ssl
...
"
    ...
    # Method 2: Run the Install Certificates command
    echo -e "${YELLOW}[2/3] Running Install Certificates command...${NC}"
    if [ -f "/Applications/Python 3.*/Install Certificates.command" ]; then
        echo "Found Install Certificates command, running..."
        /Applications/Python\ 3.*/Install\ Certificates.command
    else
        echo "Install Certificates command not found, trying alternative..."
        pip3 install --upgrade certifi
    fi
    
    # Method 3: Set environment variables
    echo -e "${YELLOW}[3/3] Setting SSL environment variables...${NC}"
    export PYTHONHTTPSVERIFY=0
    export SSL_VERIFY=false
    
else
    echo -e "${BLUE}Non-macOS system detected${NC}"
fi

echo -e "${GREEN}SSL fixes applied!${NC}"
echo ""

# Create an enhanced market data fetcher with multiple data sources
echo -e "${YELLOW}Creating enhanced market data fetcher with SSL fixes...${NC}"

cat > ssl_fixed_market_fetcher.py << 'EOF'
import urllib.request
import json
import csv
import time
...
OUTPUT_DIR="market_data_ssl_fixed_$(date +%Y_%m_%d)"
mkdir -p "$OUTPUT_DIR"
export OUTPUT_DIR="$OUTPUT_DIR"

echo -e "${GREEN}✓ SSL-fixed market data fetcher created!${NC}"
echo ""

echo -e "${CYAN}================================================================${NC}"
echo -e "${CYAN}                    STARTING ENHANCED DOWNLOADER${NC}"
echo -e "${CYAN}================================================================${NC}"
echo ""
echo -e "${YELLOW}This version will try multiple data sources:${NC}"
echo -e "${BLUE}1. Yahoo Finance (real data)${NC}"
echo -e "${BLUE}2. Investing.com (demo data)${NC}"
echo -e "${BLUE}3. Realistic demo data (always works)${NC}"
echo ""
...
python3 ssl_fixed_market_fetcher.py
...
echo -e "${CYAN}================================================================${NC}"
echo ""
echo -e "${YELLOW}📁 Check '$OUTPUT_DIR' for CSV files${NC}"
echo -e "${BLUE}Look for *_latest.csv files for most recent data${NC}"