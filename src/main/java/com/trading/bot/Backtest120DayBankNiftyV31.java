...
import com.trading.bot.ai.BankNiftyStrategyV31;
...
/**
 * Backtest120DayBankNiftyV31 — STANDALONE backtest for BankNiftyStrategyV31.
 *
 * Not integrated into AIPredictor. Run, inspect WR, decide whether to add.
 * Same checkOutcome semantics as the SENSEX fast backtest for apples-to-apples.
 */
public class Backtest120DayBankNiftyV31 {
    ...
    private static final double MIN_POINTS       = 40.0;
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("╔══════════════════════════════════════════════════════════════════════════════╗");
        System.out.println("║   BANKNIFTY V31 STANDALONE 120-DAY BACKTEST  —  NOT INTEGRATED INTO LIVE     ║");
        System.out.println("╚══════════════════════════════════════════════════════════════════════════════╝");
        ...
        if (bnf == null || bnf.size() < 500) {
            System.err.println("Insufficient BANKNIFTY data: " + (bnf == null ? 0 : bnf.size()));
            return;
        }
    ...
    private static List<Rec> run(List<SimpleMarketData> data) {
        ...
        for (int i = 250; i < data.size(); i++) {
            ...
            AIPredictor.AIPrediction p = BankNiftyStrategyV31.predict(
    ...
    private static void printReport(List<Rec> sigs) {
        System.out.println();
        System.out.println("══════════════════ BANKNIFTY V31 RESULTS ══════════════════");
        if (sigs.isEmpty()) {
            System.out.println("  No signals generated. Strategy is too strict.");
            return;
        }
        ...
        // Verdict
        System.out.println();
        if (wr >= 70) System.out.printf("  VERDICT: %.1f%% WR — ✓ ELIGIBLE for integration (≥70%% target met)%n", wr);
        else if (wr >= 65) System.out.printf("  VERDICT: %.1f%% WR — ⚠️ MARGINAL (65-70%%) — user decides%n", wr);
        else              System.out.printf("  VERDICT: %.1f%% WR — ✗ DO NOT INTEGRATE (below 65%%)%n", wr);
