...
/**
 * SENSEX Mean-Reversion V33 — TEST ONLY. SENSEX-tuned mirror of {@link BankNiftyMeanReversionV33}.
 *
 * SECONDARY entry path that fires when {@link SensexStrategyV31} returns NEUTRAL.
 * Catches RSI-extreme exhaustion bounces on choppy/ranging SENSEX days.
 *
 * Constants vs BANKNIFTY V33.3 (BANKNIFTY ~52k vs SENSEX ~78k → ~1.5× absolute scale):
 *  • MIN_SL_PTS  = 65  (BANKNIFTY: 45)
 *  • MIN_TGT_PTS = 45  (BANKNIFTY: 30)
 *  • All other parameters identical
 *
 * NOT wired into AIPredictor.
 */
public final class SensexMeanReversionV33 {

    private SensexMeanReversionV33() {}
    ...
    private static final double MIN_SL_PTS         = 65.0;
    private static final double MIN_TGT_PTS        = 45.0;
    ...
    public static AIPredictor.AIPrediction predict(
        ...
        if (recentRSIOversold && recentLowerBBTouch && bullishCandle && bodyRatio >= BODY_RATIO_MIN) {
            ...
            String reason = String.format(
                    "SENSEX_V33_MR | LONG | RSI=%.1f | BBlower | bull body=%.0f%%",
                    rsi, bodyRatio * 100);
            return new AIPredictor.AIPrediction("UP", confidence, confidence / 100.0,
                    0, rsi, atr / currentPrice, 80, "SENSEX_V33_MR", reason, tgtPts, slPts, false);
        }
        ...
        if (recentRSIOverbought && recentUpperBBTouch && bearishCandle && bodyRatio >= BODY_RATIO_MIN) {
            ...
            confidence = Math.min(95, confidence);
            String reason = String.format(
                    "SENSEX_V33_MR | SHORT | RSI=%.1f | BBupper | bear body=%.0f%%",
                    rsi, bodyRatio * 100);
    ...
    private static AIPredictor.AIPrediction neutral(double rsi, double atr, double price, String why) {
        return new AIPredictor.AIPrediction("NEUTRAL", 0, 0, 0, rsi, atr / Math.max(price, 1), 80,
                "SENSEX_V33_MR", "filtered: " + why, atr, atr, false);
    }
