...
/**
 * NIFTY Strategy V31 — high-WR trend-pullback, NIFTY-tuned mirror of {@link SensexStrategyV31}.
 *
 * Built after the V29 NIFTY 120-day backtest came in at 55.2% WR (not the 70% from the
 * cherry-picked 60-day window). Replaces the V29 path via {@code AIPredictor.USE_NIFTY_V31}
 * flag. V29 code at {@code AIPredictor.predictNiftyStrategy()} stays as instant rollback.
 *
 * Same design as SENSEX V31.1 (proven 71.1% WR) with constants retuned for NIFTY price
 * scale (~24k vs SENSEX ~80k → 1/3 the absolute points but same percentage moves).
 *
 * Constants vs SENSEX V31.1:
 *  • MIN_SL_PTS  = 20  (SENSEX: 50)
 *  • MIN_TGT_PTS = 30  (SENSEX: 75)
 *  • All multipliers, time windows, RSI bands, ADX gates IDENTICAL to SENSEX V31.1
 *  • R:R fixed 1:1.5 (SL = 0.6×ATR floor 20, Target = 1.5 × SL floor 30)
 *
 * NIFTY-specific tweaks:
 *  • Expiry handover: skip after 13:30 on TUESDAY (NIFTY weekly expiry day)
 *    [SENSEX skips THURSDAY since SENSEX expires Thu]
 */
public final class NiftyStrategyV31 {

    private NiftyStrategyV31() {}
    ...
    private static final LocalTime EXPIRY_CUTOFF = LocalTime.of(13, 30);
    ...
    /** NIFTY price scale ~24k: SL floor 20, Target floor 30. SENSEX uses 50/75. */
    private static final double MIN_SL_PTS       = 20.0;
    private static final double MIN_TGT_PTS      = 30.0;
    // V31.1: UP was 54.5% WR vs DOWN 72.2% in iter1 → tightened UP band (50-62)
    private static final double MAX_RSI_UP       = 62.0;   // V31: 65
    private static final double MIN_RSI_UP       = 50.0;   // V31: 45
    ...
    public static AIPredictor.AIPrediction predict(
        ...
        // V31.2: dropped CLOSE window — iter2 showed 14:30-15:00 had 3W/4L = 42.9% WR for NIFTY
        boolean inClose = false;
        if (!inEarly && !inPrime) return neutral(adx, rsi, atr, currentPrice, "outside V31 windows");

        // NIFTY weekly expiry: Tuesday. Skip after 13:30 (let expiry strategy own it).
        if (latest.timestamp != null
                && latest.timestamp.getDayOfWeek() == DayOfWeek.TUESDAY
                && now.isAfter(EXPIRY_CUTOFF)) {
            return neutral(adx, rsi, atr, currentPrice, "Tue post-13:30 expiry handover");
        }
        ...
        // Pullback REQUIRED (SENSEX V31.1 lesson: 4/5 confluence = 40% WR vs 5/5 = 69%)
        ...
        String reason = String.format(
                "NIFTY_V31 | conf=%d/5 | bias=%s/%s/%s | ADX=%.1f | RSI=%.1f | pullback@EMA20 | %s",
        ...
        return new AIPredictor.AIPrediction(trendDir, confidence, confidence / 100.0,
                adx, rsi, atr / currentPrice, 80, "NIFTY_V31", reason, tgtPts, slPts, true);
    }

    // ── Helpers (identical to SensexStrategyV31 — kept inline for self-containment) ──
    ...
    private static AIPredictor.AIPrediction neutral(double adx, double rsi, double atr, double price, String why) {
        return new AIPredictor.AIPrediction("NEUTRAL", 0, 0, adx, rsi, atr / Math.max(price, 1), 80,
                "NIFTY_V31", "filtered: " + why, atr, atr, false);
    }
