...
public class AIPredictor {
    ...
    private String detectCandlePattern(List<SimpleMarketData> data) {
        if (data.size() < 5) return "NONE";
        
        for (int i = 1; i <= 3; i++) {
            SimpleMarketData c1 = data.get(data.size() - i);
            SimpleMarketData c2 = data.get(data.size() - i - 1);
            
            double body1 = Math.abs(c1.price - c1.open);
            double body2 = Math.abs(c2.price - c2.open);
            double wickLower1 = Math.min(c1.price, c1.open) - c1.low;
            double wickUpper1 = c1.high - Math.max(c1.price, c1.open);

            if (wickLower1 > body1 * 1.5 && wickUpper1 < body1 * 0.5) return "BULLISH_REVERSAL";
            if (wickUpper1 > body1 * 1.5 && wickLower1 < body1 * 0.5) return "BEARISH_REVERSAL";
            if (c1.price > c1.open && c2.price < c2.open && c1.price > c2.open) return "BULLISH_ENGULFING";
            if (c1.price < c1.open && c2.price > c2.open && c1.price < c2.open) return "BEARISH_ENGULFING";
            
            double range1 = c1.high - c1.low;
            double dojiThreshold = range1 * 0.1;
            if (body1 <= dojiThreshold && wickUpper1 > body1 && wickLower1 > body1) return "INDECISION_DOJI";
            
            boolean bullishHarami = (c1.price > c1.open) && (c2.price < c2.open) && (Math.abs(c1.price - c1.open) < Math.abs(c2.price - c2.open)) &&
                                    (Math.max(c1.price, c1.open) <= Math.max(c2.open, c2.price)) &&
                                    (Math.min(c1.price, c1.open) >= Math.min(c2.open, c2.price));
            boolean bearishHarami = (c1.price < c1.open) && (c2.price > c2.open) && (Math.abs(c1.price - c1.open) < Math.abs(c2.price - c2.open)) &&
                                    (Math.max(c1.price, c1.open) <= Math.max(c2.open, c2.price)) &&
                                    (Math.min(c1.price, c1.open) >= Math.min(c2.open, c2.price));
            if (bullishHarami) return "BULLISH_HARAMI";
            if (bearishHarami) return "BEARISH_HARAMI";
        }
        
        SimpleMarketData a = data.get(data.size() - 1);
        SimpleMarketData b = data.get(data.size() - 2);
        SimpleMarketData c = data.get(data.size() - 3);
        boolean threeSoldiers = (a.price > a.open && b.price > b.open && c.price > c.open) &&
                                (a.price > b.price && b.price > c.price) &&
                                (Math.abs(a.price - a.open) > (a.high - a.low) * 0.3) &&
                                (Math.abs(b.price - b.open) > (b.high - b.low) * 0.3) &&
                                (Math.abs(c.price - c.open) > (c.high - c.low) * 0.3);
        boolean threeCrows = (a.price < a.open && b.price < b.open && c.price < c.open) &&
                             (a.price < b.price && b.price < c.price) &&
                             (Math.abs(a.price - a.open) > (a.high - a.low) * 0.3) &&
                             (Math.abs(b.price - b.open) > (b.high - b.low) * 0.3) &&
                             (Math.abs(c.price - c.open) > (c.high - c.low) * 0.3);
        if (threeSoldiers) return "BULLISH_CONTINUATION";
        if (threeCrows) return "BEARISH_CONTINUATION";
    ...
    private AIPrediction predictNiftyStrategy(String symbol, List<SimpleMarketData> data, double currentPrice, double ema50, double rsi, double adx, double atr, SimpleMarketData latest, double avgVol, OptionData optionData) {
        ...
        if (optionData != null && !finalDirection.equals("NEUTRAL")) {
            if (finalDirection.equals("UP")) {
                if (optionData.pcr > 1.1 && optionData.putOIChange > optionData.callOIChange * 1.5) finalConfidence += 4;
            } else {
                if (optionData.pcr < 0.9 && optionData.callOIChange > optionData.putOIChange * 1.5) finalConfidence += 4;
            }
            Double d = optionData.greeks != null ? optionData.greeks.get("delta") : null;
            if (d != null && Math.abs(d) >= 0.45) finalConfidence += 2;
        }
        finalConfidence = Math.min(98.0, finalConfidence);
        
        // V19 Exit: Target 0.7x ATR, SL 2.0x ATR (Ultra high probability)
        return new AIPrediction(finalDirection, finalConfidence, finalConfidence/100.0, adx, rsi, atr/currentPrice, 80, finalModel, finalReasoning, atr * 0.7, atr * 2.0, false);
    }
