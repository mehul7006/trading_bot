...
public class HonestMarketDataFetcher {
    ...
    public OptionData fetchOptionData(String symbol) {
                ...
                if (root.path("status").asText().equals("success")) {
                    ...
                    for (JsonNode strikeNode : chain) {
                        ...
                        if (!call.isMissingNode()) {
                            totalCallOI += call.path("market_data").path("oi").asDouble();
                            callOIChange += call.path("market_data").path("oi_change").asDouble();
                        }
                        if (!put.isMissingNode()) {
                            totalPutOI += put.path("market_data").path("oi").asDouble();
                            putOIChange += put.path("market_data").path("oi_change").asDouble();
                        }
                        ...
                        if (diff < minDiff) {
                            ...
                            if (!atmNode.isMissingNode()) {
                                JsonNode greeks = atmNode.path("market_data").path("greeks");
        ...
        // Fallback for demo/error: Return neutral data if API fails but spot is known
        return null;
    }
