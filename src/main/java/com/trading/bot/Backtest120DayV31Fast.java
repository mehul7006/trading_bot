...
/**
 * Backtest120DayV31Fast — lean backtest for iterating SENSEX V31.
 *
 * Bypasses the heavy AIPredictor pipeline (analyze50Plus, applyEnhancedFilters,
 * applyInstitutionalBoosts) and directly calls SensexStrategyV31.predict() with
 * pre-computed indicators. Runtime: ~30 seconds vs 10-15 min for full pipeline.
 *
 * Use this for rapid iteration in Phase 3. After hitting target WR, run
 * BacktestCombined120Days (slow, full pipeline) for final validation.
 *
 * Same checkOutcome semantics as BacktestCombined120Days for apples-to-apples comparison.
 */
public class Backtest120DayV31Fast {
    ...
    private static final double MIN_POINTS_SENSEX = 50.0;  // V31 floor
    ...
    public static void main(String[] args) throws Exception {
        AIPredictor.USE_SENSEX_V31 = true;  // ensure V31 path

        System.out.println("╔══════════════════════════════════════════════════════════════════════════════╗");
        System.out.println("║       SENSEX V31 FAST 120-DAY BACKTEST  —  Trend-Pullback Strategy          ║");
        System.out.println("╚══════════════════════════════════════════════════════════════════════════════╝");
        System.out.println("Loading 140 days SENSEX data (cached)...");

        MarketDataAgent md = new MarketDataAgent();
        List<SimpleMarketData> sensex = md.getHistoricalData("SENSEX", LOAD_DAYS);
        if (sensex == null || sensex.size() < 500) {
            System.err.println("Insufficient SENSEX data: " + (sensex == null ? 0 : sensex.size()));
            return;
        }
        System.out.println("Loaded " + sensex.size() + " candles for SENSEX.\n");

        List<Rec> signals = runSensexV31(sensex);
        printReport(signals);
    }

    private static List<Rec> runSensexV31(List<SimpleMarketData> data) {
        List<Rec> out = new ArrayList<>();

        // Determine scan range — last BACKTEST_DAYS days
        ...
        final long LOSS_COOLDOWN_MS = 90L * 60 * 1000; // 90 min after a loss, skip same direction
        ...
        for (int i = 250; i < data.size(); i++) {
            ...
            if (!cDate.equals(lastDate)) {
                lastSigMs = 0; lastUpLossMs = 0; lastDownLossMs = 0; lastDate = cDate;
            }

            long cMs = c.timestamp.atZone(ZoneId.of("Asia/Kolkata")).toInstant().toEpochMilli();
            if (cMs - lastSigMs < COOLDOWN_MS) continue;

            // Window into last 500 candles — enough for EMA200 + bucket bias
            ...
            AIPredictor.AIPrediction p = SensexStrategyV31.predict(
                    "SENSEX", hist, price, ema20, ema50, ema200, rsi, adx, atr, c);

            if ("NEUTRAL".equals(p.predictedDirection)) continue;
            if (p.confidence < MIN_CONFIDENCE) continue;
            if (p.estimatedMovePoints < MIN_POINTS_SENSEX) continue;

            // V31.1: same-direction loss cooldown — block 90 min after losing same direction
            ...
            double tgtPx  = "UP".equals(p.predictedDirection) ? price + tgtPts : price - tgtPts;
            double slPx   = "UP".equals(p.predictedDirection) ? price - slPts  : price + slPts;
    ...
    // ── Lean indicator helpers (last-N candle window) ───────────────────────
    ...
    private static void printReport(List<Rec> sigs) {
        System.out.println();
        System.out.println("══════════════════════════ V31 RESULTS ══════════════════════════");
        if (sigs.isEmpty()) {
            System.out.println("  No signals generated. Strategy is too strict — loosen filters.");
            return;
        }
        ...
        int[] hist = new int[6]; // 0,1,2,3,4,5+
        for (List<Rec> day : byDate.values()) {
            int n = day.size();
            hist[Math.min(n, 5)]++;
        }
        long tradingDays = byDate.size();
        System.out.println();
        System.out.println("  Daily call distribution (only days with ≥1 call):");
        System.out.printf("    1 call : %d days%n", hist[1]);
        System.out.printf("    2 calls: %d days%n", hist[2]);
        System.out.printf("    3 calls: %d days%n", hist[3]);
        System.out.printf("    4 calls: %d days%n", hist[4]);
        System.out.printf("    5+ call: %d days%n", hist[5]);
        System.out.printf("    Trading days with signals: %d%n", tradingDays);
        System.out.printf("    Avg calls/active day: %.2f%n", tot * 1.0 / Math.max(tradingDays, 1));

        // ── Per-call detail
        System.out.println();
        System.out.println("  Per-call detail:");
        System.out.printf("  %-12s %-6s %-5s %10s %6s %6s %5s %-12s %8s  %s%n",
                "Date", "Time", "Dir", "Entry", "+Tgt", "-SL", "Conf", "Result", "P&L", "Reason");
        System.out.println("  " + "─".repeat(120));
        for (Rec r : sigs) {
            System.out.printf("  %-12s %-6s %-5s %,10.0f %6.0f %6.0f %4.0f%% %-12s %+8.0f  %s%n",
                    r.date, r.time, r.direction, r.entry, r.tgtPts, r.slPts, r.confidence,
                    r.outcome, r.pts,
                    r.reason != null && r.reason.length() > 60 ? r.reason.substring(0, 60) + "..." : r.reason);
        }
        System.out.println();
        System.out.println("══════════════════════════════════════════════════════════════════");
    }
